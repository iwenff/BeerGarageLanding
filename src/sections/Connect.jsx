...
import { WhatsAppIcon, MaxIcon, TelegramIcon, VkIcon, StarIcon, PinIcon, ArrowIcon } from '../components/icons'
import './Connect.css'
...
const GROUPS = [
  {
    ...
    accent: true,
    actions: [
      { label: 'Руководителю в WhatsApp', href: LINKS.whatsapp, icon: WhatsAppIcon, primary: true },
      { label: 'Руководителю в MAX', href: LINKS.max, icon: MaxIcon },
    ],
  },
  {
    key: 'review',
...
export default function Connect() {
  return (
    <section className="section connect" id="connect">
      <div className="blob connect__blob" style={{ background: 'var(--amber)' }} />
      <div className="container">
        <div className="section-head connect__head">
          <span className="eyebrow" data-reveal>На связи</span>
          <h2 className="heading" data-reveal>Давай <span className="accent">на «ты»</span></h2>
          <p className="lead" data-reveal>
            Один тап — и ты с нами. Напиши руководителю, оставь отзыв или
            подпишись, чтобы не пропускать движ.
          </p>
        </div>

        <div className="connect__grid">
          {GROUPS.map((g) => (
            <article className={`cgroup ${g.accent ? 'cgroup--accent' : ''}`} data-reveal key={g.key}>
              ...
              <div className="cgroup__actions">
                {g.actions.map((a) => {
                  ...
                  return (
                    ...
                    >
                      <span className="actionbtn__ico"><Icon /></span>
